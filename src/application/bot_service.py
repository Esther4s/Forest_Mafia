...
class BotService:
    ...
    async def cancel_game(self, chat_id: ChatId, reason: str = "Игра отменена администратором") -> Dict[str, Any]:
        """Отменяет игру с полной очисткой"""
        game = self.active_games.get(chat_id.value)
        if not game:
            return {"success": False, "message": "❌ В этом чате нет активной игры."}
        ...
        if game.phase != GamePhase.WAITING:
            return {"success": False, "message": "❌ Нельзя отменить игру, которая уже началась!"}
        
        # Отменяем игру
        await self.game_service.end_game(game, None, reason)
