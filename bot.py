...
from hare_dreams import get_random_dream
...
class ForestWolvesBot:
    ...
    async def check_bot_permissions_decorator(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        game_commands = ['/join', '/start_game', '/status', '/rules', '/settings', '/end',
                        '/help', '/version', '/test_mode', '/min_players']
    ...
    async def rules(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        rules_text = (
            ...
            "• <b>Волк</b> - стая хищников, съедают по зверю каждую ночь (кроме первой ночи)\n"
            "• <b>Лиса</b> - хитрая воровка, крадет запасы еды у травоядных\n\n"
            "🐰 <b>ТРАВОЯДНЫЕ (большинство):</b>\n"
            "• <b>Заяц</b> - мирная зверушка, не имеет ночных способностей\n"
            ...
            "⏰ <b>ФАЗЫ ИГРЫ:</b>\n\n"
            "🌙 <b>НОЧЬ (60 секунд):</b>\n"
            "• Волк выбирает жертву для съедения\n"
            ...
            "🐺 <b>Волк:</b>\n"
            "• Не действует в первую ночь\n"
            "• Не может есть лису (союзник)\n"
            "• Не может есть другого волка\n\n"
    ...
    async def show_team_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            stats_text += "🐺 <b>Хищники (Волк + Лиса):</b>\n"
            stats_text += f"   🏆 Побед: {predators_wins}\n\n"
            
            stats_text += "🐰 <b>Травоядные (Заяц + Крот + Бобёр):</b>\n"
    ...
    async def award_nuts_for_game(self, game: Game, winner: Optional[Team] = None):
        ...
        try:
            ...
            # По умолчанию награды включены для всех игроков
    ...
    async def end_game(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Универсальная команда завершения игры с подтверждением"""
        chat_id = update.effective_chat.id
        user_id = update.effective_user.id

        logger.info(f"🔍 Команда /end от пользователя {user_id} в чате {chat_id}")
        ...
        if chat_id not in self.games:
            logger.info(f"🔍 Нет активной игры в чате {chat_id}")
            await update.message.reply_text("❌ Нет активной игры в этом чате!")
            return

        game = self.games[chat_id]
        logger.info(f"🔍 Найдена игра в фазе: {game.phase}, тип: {type(game.phase)}")
        logger.info(f"🔍 Игроков в игре: {len(game.players)}")
        logger.info(f"🔍 Раунд игры: {game.current_round}")
        
        # Проверяем права пользователя
        is_admin = False
        is_participant = user_id in self.player_games and self.player_games[user_id] == chat_id
        ...
        # Если пользователь не участник и не администратор
        if not is_participant and not is_admin:
            await update.message.reply_text("❌ Только участники игры или администраторы могут завершать игру!")
            return

        # Определяем тип действия в зависимости от фазы игры
        if game.phase == GamePhase.WAITING:
            action_type = "отменить"
            action_description = "отменить игру в ожидании игроков"
        else:
            action_type = "завершить"
            action_description = "завершить активную игру"
        
        # Создаем сообщение с подтверждением
        user_name = update.effective_user.first_name or update.effective_user.username or f"ID{user_id}"
        confirmation_text = (
            f"⚠️ <b>Подтверждение {action_type} игры</b> ⚠️\n\n"
            f"👤 Пользователь: {user_name}\n"
            f"🎮 Фаза игры: {game.phase.value}\n"
            f"👥 Участников: {len(game.players)}\n\n"
            f"❓ Вы действительно хотите {action_description}?\n\n"
        ...
        keyboard = [
            [
                InlineKeyboardButton("✅ Да, завершить", callback_data=f"confirm_end_game_{user_id}"),
                InlineKeyboardButton("❌ Нет, отменить", callback_data=f"cancel_end_game_{user_id}")
            ]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    async def force_end(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Если игра еще не началась (фаза ожидания), используем отмену
        if game.phase == GamePhase.WAITING:
            ...
            await self._cancel_game_internal(update, context, game, f"Игра отменена участником {admin_name}")
        else:
            # Если игра уже началась, используем завершение
        ...
        if chat_id in self.games:
            ...
            if game.phase != GamePhase.WAITING:
                await update.message.reply_text(
                    ...
                    f"Участников: {len(game.players)}\n\n"
                    "Дождитесь окончания текущей игры или используйте /end для завершения."
                )
                return
            else:
    ...
    async def _cancel_game_internal(self, update: Update, context: ContextTypes.DEFAULT_TYPE, game: Game, reason: str):
        """Внутренняя функция для отмены игры с полной очисткой"""
        try:
            # Устанавливаем флаг завершения игры
            if hasattr(game, "game_over_sent"):
                game.game_over_sent = True
            
            game.phase = GamePhase.GAME_OVER

            # Открепляем все закрепленные сообщения бота при отмене игры
            await self._unpin_all_bot_messages(context, game)

            # Отправляем сообщение об отмене игры
            message_text = f"🛑 <b>ИГРА ОТМЕНЕНА</b> 🛑\n\n📋 Причина: {reason}\n👥 Зарегистрированных игроков: {len(game.players)}\n\n🌲 Игра завершена участником, можно начинать новую игру /start!"
            ...
            try:
                ...
                logger.error(f"Не удалось отправить сообщение об отмене игры: {e}")
            ...
            # Очищаем маппинги игроков
            for pid in list(game.players.keys()):
                if pid in self.player_games:
                    del self.player_games[pid]

            # Удаляем игру из всех словарей
            if chat_id in self.games:
                del self.games[chat_id]
            if chat_id in self.night_actions:
                del self.night_actions[chat_id]
            ...
            # Очищаем состояние игры в базе данных
            if hasattr(game, 'db_game_id') and game.db_game_id:
                try:
                    from database import update_game_phase
                    update_game_phase(game.db_game_id, "cancelled", 0)
                except Exception as e:
                    logger.error(f"Ошибка обновления статуса игры в БД: {e}")
            
            # Сохраняем состояние после отмены игры
            if hasattr(self, 'auto_save_manager') and self.auto_save_manager:
                try:
                    self.auto_save_manager.force_save()
                    logger.info("✅ Состояние сохранено после отмены игры")
                except Exception as e:
                    logger.error(f"❌ Ошибка сохранения состояния после отмены игры: {e}")

            # Сообщение с кнопками уже удалено в handle_confirm_end_game_callback
                
        except Exception as e:
            logger.error(f"Ошибка в _cancel_game_internal: {e}")
            if update.callback_query:
                await update.callback_query.edit_message_text("❌ Произошла ошибка при отмене игры!")
    ...
    async def handle_welcome_buttons(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data == "welcome_start_game":
            ...
            await query.edit_message_text(
                ...
                "🎭 Роли:\n"
                "🐺 Волк (Хищник) - стая, по ночам съедает по зверю\n"
                "🦊 Лиса (Хищник) - ворует запасы еды\n"
                "🐰 Заяц (Травоядный) - мирная зверушка\n"
                "🦫 Крот (Травоядный) - роет норки, узнаёт роли других зверей\n"
                "🦦 Бобёр (Травоядный) - возвращает украденные запасы\n\n"
                "🌙 Ночные фазы: Волк → Лиса → Бобёр → Крот\n"
    ...
    async def cancel_game_from_welcome(self, query, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Используем полную очистку с информацией об администраторе
        await self._cancel_game_internal(query, context, game, f"Игра отменена участником {admin_name}")
    ...
    async def process_night_phase(self, context: ContextTypes.DEFAULT_TYPE, game: Game):
        ...
        if chat_id in self.night_actions:
            ...
            # Отправляем ЛС кроту с результатом проверки, если есть и крот жив
            if game.last_mole_check:
                mole_id = game.last_mole_check.get('mole_id')
                if mole_id and mole_id in game.players and game.players[mole_id].is_alive:
                    await self.send_mole_check_pm(context, game.last_mole_check)
                else:
                    logger.warning(f"⚠️ Крот {mole_id} мертв, не отправляем ЛС с результатом проверки")
    ...
    async def handle_confirm_end_game_callback(self, update, context: ContextTypes.DEFAULT_TYPE):
        """Обрабатывает подтверждение завершения игры"""
        try:
            query = update.callback_query
            await query.answer()
            ...
            callback_data = query.data
            if not callback_data.startswith("confirm_end_game_"):
                await query.edit_message_text("❌ Неверный callback!")
                return
            
            user_id = int(callback_data.split("_")[-1])
            
            # Проверяем, что это тот же пользователь, который инициировал завершение
            if query.from_user.id != user_id:
                await query.answer("❌ Только инициатор может подтвердить завершение игры!", show_alert=True)
                return
            ...
            # Определяем тип действия в зависимости от фазы игры
            logger.info(f"🔍 Фаза игры при отмене: {game.phase}, тип: {type(game.phase)}")
            logger.info(f"🔍 Сравнение с WAITING: {game.phase == GamePhase.WAITING}")
            
            if game.phase == GamePhase.WAITING:
                # Отменяем игру
                logger.info("✅ Используем _cancel_game_internal для отмены игры")
                user_info = query.from_user
                admin_name = user_info.first_name or user_info.username or f"ID{user_info.id}"
                await self._cancel_game_internal(query, context, game, f"Игра отменена участником {admin_name}")
            else:
                # Завершаем игру
                logger.info("✅ Используем _end_game_internal для завершения игры")
                await self._end_game_internal(query, context, game, "Участник завершил игру")
                
        except Exception as e:
            logger.error(f"Ошибка в handle_confirm_end_game_callback: {e}")
            if update.callback_query:
                await update.callback_query.answer("❌ Произошла ошибка при завершении игры!")

    async def handle_cancel_end_game_callback(self, update, context: ContextTypes.DEFAULT_TYPE):
        """Обрабатывает отмену завершения игры"""
        try:
            query = update.callback_query
            await query.answer()
            ...
            # Извлекаем user_id из callback_data
            callback_data = query.data
            if not callback_data.startswith("cancel_end_game_"):
                await query.edit_message_text("❌ Неверный callback!")
                return
            ...
            if query.from_user.id != user_id:
                await query.answer("❌ Только инициатор может отменить завершение игры!", show_alert=True)
                return
            ...
            logger.error(f"Ошибка в handle_cancel_end_game_callback: {e}")
            if update.callback_query:
                await update.callback_query.answer("❌ Произошла ошибка при отмене завершения игры!")
    ...
    def run(self):
        ...
        application.add_handler(CommandHandler("end", self.end_game))  # Универсальная команда завершения
        ...
        # Обработчики подтверждения завершения игры
        application.add_handler(CallbackQueryHandler(self.handle_confirm_end_game_callback, pattern=r"^confirm_end_game_"))
        application.add_handler(CallbackQueryHandler(self.handle_cancel_end_game_callback, pattern=r"^cancel_end_game_"))
    ...
    async def unpin_all_messages_on_startup(self):
        """Открепляет только сообщения, закрепленные ботом при старте"""
        try:
            ...
            for chat_id in authorized_chats:
                try:
                    ...
                    try:
                        pinned_message = await bot.get_chat(chat_id)
                        if pinned_message.pinned_message:
                            # Проверяем, что закрепленное сообщение отправил бот
                            if pinned_message.pinned_message.from_user and pinned_message.pinned_message.from_user.id == bot.id:
                                # Открепляем только сообщение бота
                                await bot.unpin_chat_message(chat_id, pinned_message.pinned_message.message_id)
                                unpinned_count += 1
                                logger.info(f"✅ Откреплено зависшее сообщение бота в чате {chat_id}")
                            else:
                                logger.info(f"ℹ️ В чате {chat_id} закреплено сообщение не от бота, пропускаем")
                        else:
                            logger.info(f"ℹ️ В чате {chat_id} нет закрепленных сообщений")
                    except Exception as e:
                        logger.warning(f"⚠️ Не удалось открепить сообщение в чате {chat_id}: {e}")
                        continue
    ...
    async def handle_farewell_message(self, query, context, user_id: int):
        ...
        try:
            ...
            if not can_send:
                ...
                if user_in_active_game:
                    ...
                    error_text = (
                        ...
                        f"• До начала новой игры в том же чате\n"
                        f"• Если игра завершена"
                    )
            ...
            farewell_text = (
                ...
                f"⏰ Время на прощание: до начала новой игры в чате"
            )
    ...
    async def can_send_farewell_message(self, user_id: int) -> tuple[bool, str, dict]:
        ...
        try:
            ...
            # Проверка на новую игру будет выполнена ниже в новой логике
            ...
            if last_game_chat_id in self.games:
                ...
                # Если в чате есть активная игра и пользователь в ней не участвует - это новая игра
                if current_game.phase not in ['finished', 'GAME_OVER'] and user_id not in [player.user_id for player in current_game.players.values()]:
                    logger.info(f"В чате {last_game_chat_id} уже началась новая игра, прощальное сообщение недоступно")
            ...
            try:
                ...
                # Ищем активную игру в том же чате после последней игры пользователя
                query = """
                ...
                result = fetch_query(query, (last_game_chat_id, user_id, last_game_chat_id))
                if result:
                    logger.info(f"В чате {last_game_chat_id} найдена новая игра в БД, прощальное сообщение недоступно")
                    return False, "❌ В чате уже началась новая игра! Прощальное сообщение можно отправить только до начала новой игры.", {}
                    
            except Exception as db_error:
                logger.warning(f"Ошибка проверки новой игры в БД: {db_error}")
                # Если БД недоступна, полагаемся на проверку в памяти
