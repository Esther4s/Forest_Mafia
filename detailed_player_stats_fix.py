#!/usr/bin/env python3
"""
Детальный скрипт для исправления структуры таблицы player_stats
"""
...
def detailed_fix_player_stats():
    """Детально исправляет структуру таблицы player_stats"""
    ...
    try:
        ...
        if has_player_id and not has_id:
            print(f"\n⚠️ Обнаружена старая структура с player_id, пересоздаем таблицу...")
            ...
            cursor.execute("DROP TABLE player_stats CASCADE")
            ...
            cursor.execute("DROP TABLE player_stats_backup")
            ...
            if id_type and id_type[0] != 'integer':
                print(f"⚠️ Колонка id имеет неправильный тип {id_type[0]}, исправляем...")
                # Здесь можно добавить логику для исправления типа колонки
            else:
                print(f"✅ Структура таблицы уже правильная")
...
if __name__ == "__main__":
    success = detailed_fix_player_stats()
    if success:
